...
*/
	...
	$(".efo-button").on("click",function(e){
		Validation();
...
//						.on("mouseout",function(){ $(this).css({opacity:1.0}); } );
	...
	$("*[name="+ SIDE_TOOLTIP_ZIP_NAME +"]").on("keyup",function(){
		...
		AjaxZip3.zip2addr(this,'',ZIP_INTERPOLATION_ARRAY[0],ZIP_INTERPOLATION_ARRAY[1],ZIP_INTERPOLATION_ARRAY[2],ZIP_INTERPOLATION_ARRAY[2]);	// 住所補間
		return;
	});
...
/*
				//生年月日のバリデーション
				if(REQUIRED_ITEMS[i] == "i_month"){
					var day = $("select[name=i_day]").val()
					if( $(SELECTER).val() == "4" || $(SELECTER).val() == "6" || $(SELECTER).val() == "9" || $(SELECTER).val() == "11"){
						if(day == 31) NotRequiredCount++;

					}else if($(SELECTER).val() == "2" ){
						if(day == 31 || day == 30) NotRequiredCount++;
					}
				}
*/
			}
				...
				if ($("[name="+REQUIRED_ITEMS[i]+"]:checked").val()){
					// $(SELECTER).closest("ul").css({ backgroundColor:"#FFF" });
					$(SELECTER).closest("li").css({ backgroundColor:"#FFF" });
					Delete_Tooltip(REQUIRED_ITEMS[i]);
				}else{
					// $(SELECTER).closest("ul").css({ backgroundColor:FORM_BG_COLOR });
					$(SELECTER).closest("li").css({ backgroundColor:FORM_BG_COLOR });
					NotRequiredCount++;
					if(tooltip_place_name == REQUIRED_ITEMS[i]) Display_Tooltip(tooltip_place_name,"未選択です");
					required_text += "「" + $("#quoted_"+REQUIRED_ITEMS[i]).html() + "」";
				}
...
*/
					...
					case "i_mail":
						if( $(SELECTER).val().length == 0 ){
							...
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}else if ( $(SELECTER).val().match(/^[_a-zA-Z0-9-]+([\.\+_a-zA-Z0-9-]+)*@[a-zA-Z0-9]([_a-zA-Z0-9-])*(\.[_a-zA-Z0-9-]+)*(\.[_a-zA-Z0-9-]{2,})+$/i) == null ) {
        					$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}else{
							$(SELECTER).css({ "backgroundColor":"#FFF" });
						...
						break;
					case "i_zip":
						if ( $(SELECTER).val().replace(/-/g,"").replace(/ー/g,"").replace(/―/g,"").replace(/－/g,"").replace(/‐/g,"").replace(/－/g,"").replace(/‐/g,"").match(/^[0-9０-９]+$/) == null ) {
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}else if ( $(SELECTER).val().replace(/-/g,"").replace(/ー/g,"").replace(/―/g,"").replace(/－/g,"").replace(/‐/g,"").replace(/－/g,"").replace(/‐/g,"").length != 7 ) {
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}
						else{
					...
					case "i_card":
						if ( $(SELECTER).val().replace(/-/g,"").replace(/ー/g,"").replace(/―/g,"").replace(/－/g,"").replace(/‐/g,"").replace(/－/g,"").replace(/‐/g,"").match(/^[0-9０-９]+$/) == null ) {
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}else if ( $(SELECTER).val().replace(/-/g,"").replace(/ー/g,"").replace(/―/g,"").replace(/－/g,"").replace(/‐/g,"").replace(/－/g,"").replace(/‐/g,"").length != 16 ) {
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}
						else{
						...
						break;
					case "i_code":
						if ( $(SELECTER).val().match(/^[0-9０-９]+$/) == null ) {
							$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
							NotRequiredCount++;
						}else{
							$(SELECTER).css({ "backgroundColor":"#FFF" });
...
//			$("#submit_button").html("<p class='next'>未入力の項目があります</p>");
//			$("#submit_button").html("<a class='efo-button'><p class='next'>未入力の項目があります</p></a>");
//			$("#submit_button").html("<p class='next'><input type='button' class='efo-button'>未入力の項目があります</p>");
			$("#"+BTNTXT_NOTICE).css("display","inline");
			$("#"+BTNTXT_COMPLETE).css("display","none");
			isBtnDisable = true;
		}else if(NotrequiredCount < 1 && isBtnDisable){
//			$("#submit_button").html("<p class='next active'><input type='submit'>確認ページに進む</p>");
