...
$(document).ready(function(){
	...
	//【DEFINE】========================================================================//
		...
		/*                 ▼          設置ページ上部で宣言             ▼                   */
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			// 必須項目の<input>タグのnameプロパティ指定 //
			var REQUIRED_ITEMS = new Array("i_name","i_name_kana","i_pref","i_add1","i_add2","i_tel");
			// カナ変換TRUEかFALSEか //
			var KANA_CONVERSION = "TRUE";
			// カナ変換の入出力フォームnameプロパティ指定 //
			var KANA_CONVERSION_NAME = new Array("i_name","i_name_kana");
			// 住所補間で値を挿入する要素のnameプロパティ
			var ZIP_INTERPOLATION_ARRAY = new Array("i_pref","i_add1","i_add2");
			// 吹き出しの色を指定
			var SIDE_TOOLTIP_COLOR = "#000";
			// textareaに初期状態でコメントを入れておく為のnameプロパティとコメント指定
			var TEXTAREA_COMMENT_NAME = "i_contents";
			var TEXTAREA_COMMENT = "その他ご意見･ご要望などございましたら、こちらにご記入ください。";
			// 郵便番号用の吹き出しを出すnameプロパティ
			var SIDE_TOOLTIP_ZIP_NAME = "i_postnum";
			// 入力完了前のツールチップ文言 //
			var TOOLTIP_NOTICE_TEXT = "<span id='efo_box' style='font-weight:bold; color:#ffb700; cursor:pointer; text-decoration:underline;'>必須項目</span>を入力して下さい。";
			// 入力完了後のツールチップ文言 //
			var TOOLTIP_COMPLETE_TEXT = "<span style='font-weight:bold; color:#ffbb99;'>入力が完了しました。</span><br />確認画面にお進みできます。";
			// 入力完了前のツールチップ背景色 //
			var TOOLTIP_NOTICE_COLOR = "#ff2000";
			// 入力完了後のツールチップ背景色 //
			var TOOLTIP_COMPLETE_COLOR = "#44bbcc";
			// ツールチップ透明度 //
			var TOOLTIP_OPACITY_RATE = "0.7";
			// 未入力時のボタンの画像パス //
			var IMG_NOTICE   = "img/b_efo.png";
			// 完了時のボタンの画像パス //
			var IMG_COMPLETE = "img/b_entry.png";
			// 必須項目の背景色 //
			var FORM_BG_COLOR = "#FFC8C8";
			// フォーム点滅色 //
			var FORM_BG_FLASH_COLOR = "#FFD700";
		~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	...
	//【EVENT ACTION】=================== バリデーションを走らせるイベント
		...
		$("input:not(#confirm_button)").keyup(function(){ Rewrite_Box( Validation() ); });
		$("input:not(#confirm_button) , select").blur(function(){ Rewrite_Box( Validation() ); });
		$("input:not(#confirm_button) , select").change(function(){ Rewrite_Box( Validation() ); });
		$("input:not(#confirm_button) , select").focusout(function(){ Rewrite_Box( Validation( $(this).attr("name") ) ); });
	...
	$(document).on("click", "[name=button]:not(.check)" ,function(){
		Validation();
	...
	$(document).on("mouseup","#confirm_button",function(){
		CancelBeforeUnload();
	...
	function Validation(tooltip_place_name){	// 引数にはtooltipを吐き出す要素のnameプロパティを指定か、空か
		...
		for( var i=0;i < REQUIRED_ITEMS.length;i++ ){
			...
			if ($(SELECTER).attr("type") == "radio"){
				// value=0(未選択)の時だけ、背景色を変える
				if (!$(SELECTER+":checked").val()){
					$("#building clearfix").css({ backgroundColor:FORM_BG_COLOR });
					/*$(SELECTER).css({ backgroundColor:FORM_BG_COLOR });*/
					NotRequiredCount++;
					/*if(tooltip_place_name == REQUIRED_ITEMS[i]) Display_Tooltip(tooltip_place_name,"未選択です");*/
				}else{
					$("#building clearfix").css({ backgroundColor:"#FFF" });
					$(SELECTER).css({ backgroundColor:"#FFF" });
					/*Delete_Tooltip(REQUIRED_ITEMS[i]);*/
				}
			}
				...
				/*-------------------------------固有バリデーション処理*/
					switch(REQUIRED_ITEMS[i]){
						...
						// case "i_gender":
						// /*ここにtel固有の処理*/
						// var genderCheckedFlag = false;

						// if($(SELECTER).prop('checked')){
						// 	genderCheckedFlag = true;
						// }
						// if( !genderCheckedFlag ){
						// 	$(SELECTER).css({"backgroundColor":FORM_BG_COLOR});
						// 	NotRequiredCount++;
						// 	if(tooltip_place_name == REQUIRED_ITEMS[i]) Display_Tooltip(tooltip_place_name,"未入力です");
						// 	// required_text += "「" + $("#quoted_"+REQUIRED_ITEMS[i]).html() + "」";
			...
			//フォーム最下部にある未入力リストのテキスト生成
			if(NotRequiredCount > 0){
				/*$(".error").html("<p>未入力の必須項目があります。<br />" + required_text + "は必須項目ですので、ご入力ください。</p>")
				                .show();*/
			}else{
				$(".error").hide();
			}
	...
	function Rewrite_Box(NotrequiredCount){
		...
		if(NotrequiredCount >= 1){
			$("input[name=button]").val("未入力の項目があります");
			$("input[name=button]").removeClass('check');
		}else{
			$("input[name=button]").val(BUTTON_MESSAGE);
			$("input[name=button]").addClass('check');
		}
		return;
	}

	if( $("#form1").submit( function (){
		if( !$("input[name=button]").hasClass('check') ){
			return false;
		}
	}) );
