...
namespace Plugin\CloseProductRedirect\HookPoint\Front;
...
class ProductDetailService extends HookBaseService
{
     ...
     * リダイレクト先商品の場合の処理
     *
     * @param \Plugin\CloseProductRedirect\Entity\CprProductRedirect $CprProductRedirect
    ...
    private function checkRedirectUrl($CprProductRedirect, &$redirect_url, &$redirect_product_id, &$redirect_action){
        ...
        if ($isUrl){
            ...
            $qb->select('p.id, pr.redirect_select, pr.redirect_url')
                ->leftJoin('Plugin\CloseProductRedirect\Entity\CprProductRedirect', 'pr', 'WITH', 'p.id = pr.id')
