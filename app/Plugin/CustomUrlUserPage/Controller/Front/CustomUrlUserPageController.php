...
class CustomUrlUserPageController extends AbstractController
{
     ...
     */
    public function index(Application $app, Request $request,$indextype)
    {
        $pagination = null;
    ...
    public function edit(Application $app, Request $request, $id)
    {
        ...
        // IDからおすすめ商品情報を取得する
        $CustomUrlUserPage = $app['eccube.plugin.recommend.repository.recommend_product']->findById($id);
        ...
        $form = $app['form.factory']
            ->createBuilder('admin_recommend', $CustomUrlUserPage)
            ->getForm();
    ...
    public function rankUp(Application $app, Request $request, $id)
    {
        ...
        // IDからおすすめ商品情報を取得する
        $CustomUrlUserPage = $app['eccube.plugin.recommend.repository.recommend_product']->find($id);
        if (is_null($CustomUrlUserPage)) {
            $app->addError('admin.recommend.notfound', 'admin');
            return $app->redirect($app->url('admin_recommend_list'));
        }
