...
class MakerRelatedProductController
{
    ...
    public function index(Application $app, Request $request)
    {
        ...
        if( isset($_SESSION['ec_save_pr_id']) ){
            ...
            $setting = $app['maker_related_product_setting.repository.maker_related_product_setting']
            ...
            $query = $app['orm.em']
                ->createQuery(
                    'SELECT count(rp.id) cn FROM Plugin\MakerRelatedProduct\Entity\MakerRelatedProduct rp'
                );
            $rs = $query->getResult();
            if($rs[0]['cn'] > $max_row){
                // 古いrowを削除
                ...
                $stmt = $app['orm.em']->getConnection()->prepare('delete from plg_maker_related_product order by updated_at asc limit ' . $del_rows);
                $stmt->execute();
            }
            ...
            if($_from_id != $id){
                $rp_obj = new MakerRelatedProduct();
            ...
            $stmt = $app['orm.em']->getConnection()->prepare('
                    SELECT count(rp.to_id) cn, rp.to_id FROM plg_maker_related_product rp, dtb_product as p WHERE rp.from_id=' . $id . ' AND rp.to_id = p.product_id AND p.del_flg = 0 AND p.status = 1 GROUP BY rp.from_id, rp.to_id ORDER BY cn DESC
                    ');
            $stmt->execute();
            $rs = $stmt->fetchAll();
            ...
            if(strlen($or_str) > 1) {
                $stmt = $app['orm.em']->getConnection()->prepare('
                    SELECT ecp.product_id, ecp.name, ecp.description_detail, ecpi.file_name, (select MIN(in_pcl.price02) FROM dtb_product_class in_pcl WHERE in_pcl.product_id = ecp.product_id AND in_pcl.del_flg <> 1 GROUP BY in_pcl.product_id) min_price, (select MAX(in_pcl.price02) FROM dtb_product_class in_pcl WHERE in_pcl.product_id = ecp.product_id AND in_pcl.del_flg <> 1 GROUP BY in_pcl.product_id) max_price FROM dtb_product ecp, dtb_product_image ecpi WHERE ' . $or_str );
                $stmt->execute();
